...
int main() {
	...
	unsigned char* left_resized_image;
	unsigned char* right_resized_image;
	unsigned output_width, output_height;

	unsigned char* left_filtered_image;
	unsigned char* right_filtered_image;

	float* left_grayscaled_image;
	float* right_grayscaled_image;

	unsigned char* left_bordered_image;
	unsigned char* right_bordered_image;
	unsigned bordered_width, bordered_height;
	
	// 260 -> from the calib.txt
	int max_disp = 260 / SCALE_RATE;


	//creating sections which can be executed parallelly
	#pragma omp parallel
	{
		#pragma omp sections
		{
			#pragma omp section
			{
				...
				// resizing the left image
				output_width = width / SCALE_RATE;
				output_height = height / SCALE_RATE;
				left_resized_image = resize_image(left_image, height, width);

				//grayscaling left image
				left_grayscaled_image = grayscale_image(left_resized_image, output_height, output_width);

				//filtering the left image using gaussian blur
				left_filtered_image = filter_image(left_grayscaled_image, output_height, output_width);
				
				// adding border to the left image so zncc algorithm can work properly
				bordered_width = output_width + 2 * max_disp;
				bordered_height = output_height + 2 * max_disp;
				left_bordered_image = add_border(left_filtered_image, max_disp, output_height, output_width);
			}
			#pragma omp section
			{
				//loading the right image
				...
				// resizing the right image
				output_width = width / SCALE_RATE;
				output_height = height / SCALE_RATE;
				right_resized_image = resize_image(right_image, height, width);

				//grayscaling right image
				right_grayscaled_image = grayscale_image(right_resized_image, output_height, output_width);

				//filtering the right image using gaussian blur
				right_filtered_image = filter_image(right_grayscaled_image, output_height, output_width);
				
				// adding border to the right image so zncc algorithm can work properly
				bordered_width = output_width + 2 * max_disp;
				bordered_height = output_height + 2 * max_disp;
				right_bordered_image = add_border(right_filtered_image, max_disp, output_height, output_width);
	...
	// waiting for all sections to finish
	#pragma omp barrier
	...
	//post-processing including cross-check and occlusion filling
	...
	save_image(cross_checked_image, output_filename, output_height, output_width);
...
unsigned char* calc_zncc(unsigned char* image_left, unsigned char* image_right, unsigned max_disp, unsigned height, unsigned width, bool left_first) {
	...
	// parallelization of the outer for loop and setting max_zncc and best_disp to private (without it there are some synchronisation issues)
	// can't parallelize the inner loops because it depends on the result from the previous iteration
	#pragma omp parallel for private(max_zncc, best_disp)
	...
	end_profiling(start, "ZNCC calculation");
	return disp_image;
}
...
void normalize_image(unsigned char* image,unsigned height, unsigned width, unsigned max_disp) {
	...
	// Parallelizing the for loop by collapsing the nested loops
	...
	end_profiling(start, "Normlaization");
}
...
unsigned char* cross_check_image(unsigned char* left_disp_image, unsigned char* right_disp_image, unsigned height, unsigned width, unsigned threshold) {
	...
	end_profiling(start, "Crosschecking");
	return cross_image;
}
...
void occlusion_fill_nearest(unsigned char* disp_image, unsigned height, unsigned width) {
	...
	// parallelization of the for loop using schedule(static) -> creates equal sized iteration chunks and assigns them to the threads created (dynamic and guided can be also used no significant difference)
	#pragma omp parallel for schedule(static)
	for (int row = 0; row < height; row++) {
		for (int col = 0; col < width; col++) {
					...
					for (int fil_row = -win_size; fil_row <= win_size; fil_row++) {
						for (int fil_col = -win_size; fil_col <= win_size; fil_col++) {
							...
							if (pos_row >= 0 && pos_row < height && pos_col >= 0 && pos_col < width) {
								int fil_idx = pos_row * width + pos_col;
...
unsigned char* resize_image(unsigned char* image, unsigned height, unsigned width) {
	...
	/* Image resizing */
		// parallelization of the for loop using schedule(static) -> creates equal sized iteration chunks and assigns them to the threads created (dynamic and guided can be also used)
	#pragma omp parallel for schedule(static)
